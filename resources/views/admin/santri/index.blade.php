...
<div class="bg-white rounded-2xl shadow-lg p-4 mx-6 relative z-40 mb-8 -mt-[80px]">
    <!-- Konten -->
    <div class="p-6 bg-white rounded-2xl">

        <!-- Header + Search + Tombol -->
        <div class="flex items-center justify-between mt-[85px] mb-4">
            ...
            <div class="flex items-center gap-2]">
                ...
                <form action="{{ route('santri.index') }}" method="GET" class="flex">
                    <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari santri..."
                        class="border border-gray-300 rounded-lg px-3 py-2 w-80 text-1xl focus:outline-none focus:ring-2 focus:ring-primary">
                    <button type="submit" class="ml-2 px-3 py-3 bg-gray-200 rounded-lg hover:bg-gray-300">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-500" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                ...
                class="bg-primary hover:bg-green-700 text-white ml-3 px-4 py-2 rounded-lg text-1xl font-medium flex items-center gap-2">
                    + Tambah
                </a>
            </div>
        </div>
            ...
            <table class="w-full border border-gray-300 text-sm text-left rounded-lg overflow-hidden">
                <thead class="bg-gray-200">
                    <tr>
                        ...
                        <th class="border px-3 py-2">Nama Santri</th>
                        <th class="border px-3 py-2">L/P</th>
                        <th class="border px-3 py-2">Tanggal Lahir</th>
                        <th class="border px-3 py-2">Wali Santri</th>
                        <th class="border px-3 py-2">Alamat</th>
                        <th class="border px-3 py-2">No. Telepon</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="border px-3 py-2">{{ $no+1 }}</td>
                        <td class="border px-3 py-2">{{ $s->nama }}</td>
                        <td class="border px-3 py-2">{{ $s->jenis_kelamin }}</td>
                        <td class="border px-3 py-2">{{ $s->tanggal_lahir }}</td>
                        <td class="border px-3 py-2">{{ $s->wali }}</td>
                        <td class="border px-3 py-2">{{ $s->alamat }}</td>
                        <td class="border px-3 py-2">{{ $s->telepon }}</td>
                        <td class="border px-3 py-2 flex gap-2">
                            <!-- Tombol Edit -->
                            <a href="{{ route('santri.edit', $s->id) }}" class="text-blue-500 hover:text-blue-700">✏️</a>
                            <!-- Tombol Hapus -->
